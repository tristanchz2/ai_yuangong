...
async function main() {
  ...
  if (mode === 'latest') {
    ...
    console.log(`  API 共 ${result.totalNum} 条，当前页 ${result.rows.length} 条`);
  ...
  allItems.forEach((r, i) => {
    console.log(
      `  ${i + 1}. [${r.projTypeName}] ${r.noticeTitle.substring(0, 50)}... (${formatIssueDate(r.issueDate)})`
    );
  });
}
