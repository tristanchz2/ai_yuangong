...
async def extract_batch(
    ...
    if sub_keywords:
        ...
        sub_prompt_section = (
            f"\n\n★ 订阅词匹配（严格模式）：用户订阅的关键词列表：【{words_list}】"
            f"\n判断规则：订阅词必须是该公告的『采购标的/核心需求』，即这个项目就是要买这个东西或做这件事。"
            f"\n严格禁止过度匹配："
            f"\n  - 正文中仅仅提及关键词（如流程性要求、附带条件）不算相关"
            f"\n  - 例如：关键词'发票数据'，只有采购标的就是购买发票数据时才匹配；装修项目里提到'需开发票'不算相关"
            f"\n  - 例如：关键词'云计算'，只有采购云计算服务/设备时才匹配；普通IT项目提到'部署在云上'不算相关"
            f"\n  - 标题直接包含订阅词且语义一致时，可判定为相关"
            f"\n在每条结果中额外添加 \"subscription_matches\" 字段，"
            f"key 为订阅词，value 为 1（采购标的相关）或 0（不相关）。"
        ...
        try:
            response = await client.chat.completions.create(
                ...
                messages=[
                    {
                        "role": "system",
                        "content": "你是一个专业的公告信息提取助手。数据来源包含多个银行/机构采购平台。请根据正文内容判断公告类型：变更/更正/延期/澄清属于采购公告(0)，废标/终止/流标/失败/撤销属于其他(2)，非招标采购类文档也归为其他(2)。请严格按照要求的 JSON 格式输出。只输出 JSON，不要输出其他内容。",
                    },
